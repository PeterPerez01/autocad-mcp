...
class AutoCADCOMServer:
    ...
    def _iter_modelspace_entities(self):
        """Yield entities from model space safely."""
        model_space = self.doc.ModelSpace
        for i in range(model_space.Count):
            try:
                yield model_space.Item(i)
            except Exception:
                continue

    def _delete_entities_with_filter(self, predicate) -> Dict[str, Any]:
        """Delete entities matching predicate and return summary."""
        if not self.ensure_connection():
            return {"error": "Not connected to AutoCAD"}

        deleted = []
        for entity in list(self._iter_modelspace_entities()):
            try:
                if predicate(entity):
                    handle = str(entity.Handle)
                    entity.Delete()
    ...
    def delete_entities_by_handles(self, handles: List[str]) -> Dict[str, Any]:
        wanted = {str(h) for h in handles}
        result = self._delete_entities_with_filter(lambda entity: str(entity.Handle) in wanted)
        result["requested_count"] = len(wanted)
        return result

    def delete_entities_by_type(self, entity_type: str) -> Dict[str, Any]:
        return self._delete_entities_with_filter(lambda entity: str(entity.ObjectName) == entity_type)

    def delete_entities_by_layer(self, layer_name: str) -> Dict[str, Any]:
        return self._delete_entities_with_filter(lambda entity: str(entity.Layer) == layer_name)

    def delete_entities_by_color(self, color) -> Dict[str, Any]:
        color_index = self.get_color_index(color)
        return self._delete_entities_with_filter(lambda entity: int(entity.Color) == int(color_index))

    def delete_entities_by_type_and_color(self, entity_type: str, color) -> Dict[str, Any]:
        color_index = self.get_color_index(color)
        return self._delete_entities_with_filter(
            lambda entity: str(entity.ObjectName) == entity_type and int(entity.Color) == int(color_index)
        )

    def delete_last_entities(self, count: int = 1) -> Dict[str, Any]:
        if not self.ensure_connection():
            return {"error": "Not connected to AutoCAD"}
        ...
        try:
            ...
            to_delete = max(0, min(int(count), total))
            deleted = []

            for i in range(total - 1, total - to_delete - 1, -1):
                try:
                    entity = model_space.Item(i)
                    deleted.append(str(entity.Handle))
                    entity.Delete()
                except Exception:
                    continue
            ...
            return {"error": f"Failed to delete last entities: {str(e)}"}

    def delete_all_entities(self, confirm: bool = False) -> Dict[str, Any]:
        if not confirm:
            return {"error": "Deletion not confirmed. Set confirm=true."}
        return self._delete_entities_with_filter(lambda _: True)
    ...
    def undo_last_operation(self) -> Dict[str, Any]:
        ...
        try:
            self.doc.SendCommand("._UNDO _1 ")
            return {"success": True, "message": "Undo command sent"}
        except Exception as e:
            return {"error": f"Failed to undo: {str(e)}"}

    def change_entity_color(self, handle: str, color) -> Dict[str, Any]:
        if not self.ensure_connection():
            return {"error": "Not connected to AutoCAD"}

        color_index = self.get_color_index(color)

        for entity in self._iter_modelspace_entities():
            try:
                if str(entity.Handle) == str(handle):
                    entity.Color = int(color_index)
                    self.doc.Regen(constants.acActiveViewport)
                    return {
                        "success": True,
                        "handle": str(handle),
...
def run():
    """Synchronous entrypoint used by console scripts."""
    asyncio.run(main())
